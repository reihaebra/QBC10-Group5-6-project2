    ...
    <table className="w-fit font-normal text-base">
      <thead className="border-b border-input-light dark:border-[var(--color-input-dark)]">
        <tr>
          ...
          <th className="py-2 text-right pl-20 pr-2">نام محصول</th>
          <th className="py-2 px-8 text-right">تعداد</th>
          <th className="py-2 px-10 text-right">قیمت</th>
          <th className="py-2 px-15 text-right w-fit">قیمت نهایی</th>
        </tr>
      </thead>
      <tbody>
        {orderItems.map(({ _id, image, name, qty, price }) => (
          <tr key={_id} className="align-middle">
            <td className="py-3">
              <img
                className=" h-16 w-16 "
                src={"https://qbc9.liara.run/uploads/" + image}
                alt={name}
              />
            </td>
            <td className="pr-2 py-3">{name}</td>
            <td className="py-3 text-center">{qty}</td>
            <td className="py-3 text-center ">{formatMoney(price)}</td>
